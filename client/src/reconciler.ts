...
import { AuditSink, InMemoryAuditSink, makeAuditStep } from "./audit.js";
import { PublicKey } from "./domain.js";
import { RegistryEventSink, makeRegistryEvent, snapshotToRefreshEvent } from "./events.js";
...
export class RegistryReconciler {
  ...
  private readonly targets: readonly ReconciliationTarget[];
  private readonly checkpoint?: FileCheckpointRepository;
  private readonly auditSink: AuditSink;
  private readonly backoff: BackoffPolicy;
  private readonly clock: ReconcilerClock;
  ...
  async runOnce(): Promise<ReconciliationResult> {
    ...
    const status: ReconciliationStatus = requiredFailure ? "failed" : "succeeded";
  ...
  async runMany(count: number): Promise<readonly ReconciliationResult[]> {
  ...
  private async refreshWithRetry(target: ReconciliationTarget): Promise<{
    readonly attempts: number;
  ...
  private async audit(name: string, status: "started" | "accepted" | "failed", context: Record<string, unknown>): Promise<void> {
    await this.auditSink.record(makeAuditStep({ name, status, context }));
  }
}

export function makeRegistryReconciler(options: RegistryReconcilerOptions): RegistryReconciler {
  return new RegistryReconciler(options);
}

export function normalizeBackoff(input: Partial<BackoffPolicy> = {}): BackoffPolicy {
...
export function nextDelayMs(policy: BackoffPolicy, failedAttempt: number): number {
...
function makeFailure(issuer: PublicKey, attempt: number, cause: unknown, retryable: boolean): ReconciliationFailure {
  return {
    issuer,
    attempt,
    retryable,
